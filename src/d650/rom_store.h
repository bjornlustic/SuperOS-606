// rom_store.h -- user-supplied D650C mask ROM: EEPROM persistence + the
// SysEx receiver (combined build only).
//
// The firmware ships WITHOUT the copyrighted D650C mask ROM. The user dumps
// their own chip (or uses a dump they are legally entitled to) and uploads it
// over DIN MIDI in the RE-303/recpu nibble format, the same encoding
// tools/d650/syx2rom.py used to decode:
//
...
// (UCOM4_ROM_RD is redirected by D650_ROM_IN_RAM in the combined build; a RAM
// fetch is a cycle cheaper than the LPM it replaces, so the interpreter's
// timing margin is preserved).
#pragma once
...
// from the upload paths, petting the 8 s loop watchdog as it goes. The magic
// is invalidated first and rewritten last.
